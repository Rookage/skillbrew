...
from skillbrew import record as Rec  # noqa: N812


def _fake_g(*, installed, sessions, rec=None, forms=None, descs=None, resolve_trace=None):
    ...
    for n in installed:
        by_name[n] = {
            "display_name": n,
            "description": (descs or {}).get(n, f"Use when user wants {n}."),
        }
...
def test_trigger_use_when():
    ...
    t = Rec._trigger("Generate designs. Use when user wants to design an API, explore options.")
...
def test_trigger_fallback_sentence():
    ...
    t = Rec._trigger("Turn a loose idea into a map. Drive them to resolution.")
    assert t == "Turn a loose idea into a map", f"应取首句: {t!r}"
    t2 = Rec._trigger("先做这个。再做那个。")
...
def test_invoke_hint_by_form():
    ...
    s = Rec._invoke_hint("foo", "Skill")
    assert "foo" in s and "自动加载" in s, f"Skill: {s}"
    assert "暴露工具" in Rec._invoke_hint("x", "MCP"), "MCP 应说暴露工具"
    assert "代码" in Rec._invoke_hint("x", "代码"), "代码形态"
    assert "设置" in Rec._invoke_hint("x", "配置"), "配置形态"
    assert "自动加载" in Rec._invoke_hint("x", None), "form 缺省当 Skill"
...
def test_section_real_install():
    ...
    out = "\n".join(Rec._d22_invoke_section(g, heading="## 8. 怎么调用\n"))
    ...
    rows = [
        ...
        if ln.startswith("| ") and "ask-matt" in ln or "review" in ln or "obsidian-vault" in ln
    ]
...
def test_section_with_recommend():
    ...
    g = _fake_g(
        installed=["a", "b"], sessions=[{"id": 1}], rec={"mode": "keyword", "approved": ["a"]}
    )
    out = "\n".join(Rec._d22_invoke_section(g, heading="## 8. 怎么调用\n"))
...
def test_provenance_label_all_values():
    ...
    assert Rec._provenance_label("cache") == "缓存命中"  # 对齐 installer.py cache_lookup 写的值
    assert Rec._provenance_label("catalog") == "catalog 种子"
    assert Rec._provenance_label("ai") == "AI 推断（已验证）"
    assert Rec._provenance_label("ai_unverified") == "AI 推断（未验证）"
    assert Rec._provenance_label("unresolved") == "未解析"
    assert Rec._provenance_label("bogus") == "bogus"  # 未知值回退原文
    assert Rec._provenance_label("") == "未知"  # 空字符串→未知
    assert Rec._provenance_label(None) == "未知"  # None→未知
    ...
    rt = {
        ...
        "unresolved": [
            {"name": "ghost", "reason": "试跑失败", "missing": ["API_KEY"]},
            {
                "name": "shadow",
                "reason": "catalog 无此条目",
                "missing": ["repo_url", "install_command"],
            },
        ],
    }
...
def test_section_empty_unresolved_no_fallback():
    ...
    g = _fake_g(
        ...
        sessions=[{"id": 1}],
        resolve_trace={
            "items": {"a": {"provenance": "ai", "trace": [], "missing": []}},
            "unresolved": [],
        },
    )
