...
- 脱敏三关全过：① 两把真 key（DeepSeek len=35 / Agnes len=51）零痕迹（不在任何 data/registry.db）；② B站 cookie 靠 data/ 整排解决；③ 本机路径（registry.db 54 条 `~/.coze`）靠 data/ 整排解决，HANDOFF.md + `scripts/register_install_a.py` 的硬编码本机路径改通用（`~/.claude/skills`、`<workspace>/.claude/skills`、`ROOT.parent.parent/.claude/skills`）。
...
- **用户级（不可见）** = `~/.claude/skills/`（matt-pocock 14 个下在这，本环境不加载）。
...
### 2.3 dedup.json
- `baseline.skill_dirs` = `["~/.claude/skills", "<workspace>/.claude/skills"]`
...
```bash
cd ai-self-evolution/skillbrew
