...
一个 **AI 能力包管理器**：把视频 / 报告 / 文件 / 网页丢进去，自动消化成「AI 可执行计划」，授权后按步安装成 AI 的真实能力（Skill / MCP / 代码 / 配置），并维护一张可去重、可累加、可视化的能力台账。开源，本地优先。
...
| **能力产物 Artifact** | 计划落地后的实物：Skill / MCP / 代码 / 配置 / Prompt。形态由计划内容决定，不预设 | 成品酒 |
...
| ⑦ 安装 | 按步落地为能力产物(Skill/MCP/代码/配置) → 写入台账 | 授权门坐这(D22) |
...
> **关键设计**：形态由计划内容决定（D2），不预设。install_method 由源形态决定（D20），不硬编码 copy_whole_dir。
...
- **2026-06-21 r25**：**文档全面增强（品牌化 + 架构可视化 + 里程碑更新）**。① **§1 核心概念**：术语表加「类比」列（酿造隐喻：葡萄→原料、发酵→消化、配方→计划、成品酒→产物、酒窖清单→台账、品鉴笔记→记录、酒窖全景图→看板、产地认证→溯源）+ 酿造隐喻说明块。② **§3 8 步管线**：从 10 步精简为 8 步（采集→理解→消化→溯源→判断→去重→安装→记录），加表格说明每步输入/输出/关键设计。③ **§5.4 能力产物落地**：加表格（Skill/MCP/Code/Config/Prompt 五种形态的落地位置/安装方式/示例）+ 关键设计说明（D2/D20）。④ **§7 架构**：从简单目录结构扩展为 4 小节（7.1 目录结构 + 7.2 8 步管线数据流图 + 7.3 关键模块映射表 + 7.4 分支决策点），可视化展示数据流和模块职责。⑤ **§8 框架/依赖**：从单段描述扩展为 5 小节（8.1 技术栈总览表 + 8.2 核心依赖表 + 8.3 可选依赖 + 8.4 云电脑环境坑 + 8.5 Agnes 接入速查），技术栈一目了然。⑥ **§9 里程碑**：从 3 行扩展为 3 小节（9.1 已完成 MVP/v1 + 9.2 规划中 v2/v3+ + 9.3 延后需求），标注已完成功能 + 已真装 4 个 MCP 服务器。⑦ **§10 开放问题**：从 6 条混合状态改为 2 小节（✅已解决 5 条 + 🔄待探索 3 条），历史决策清晰可追。**纯文档增强，未改任何决策、未改代码**。

- **2026-06-23 r26**：**文档与落地页同步到真实现状**。① 修正 README/CHARTER 中 MCP 数量与 GitHub PAT 状态：从「已真装 4 个 MCP / GitHub 待凭证」更新为「已真装 5 个 MCP / GitHub PAT 已配置并通过冒烟测试」。② 明确 memos MCP 与 MoneyPrinterTurbo 为「已和你确认暂不安装」，避免 README 继续把它们算成「已完成」。③ 把五大核心能力品牌化写入 README：🧠 自动去重、⭐ 自动评估、⚡ 真实安装、📊 透明台账、🖥️ 双运行时中立，并成为顶部导语。④ 删除硬编码动态数字（如「装了 56 个能力」「4 个 MCP」），改为「具体数字以本地台账为准，不硬编码」。⑤ 在 `pyproject.toml` 增加 `repository` 与 `homepage` URL。⑥ 新建对外落地页 `docs/index.html` 与 `.github/workflows/pages.yml`，通过 GitHub Pages 自动部署。⑦ 为后续「每次 commit 同步维护文档」立下惯例。**纯文档/门面工程，未改动业务代码、未触碰 `.env` 与 `data/`**。
