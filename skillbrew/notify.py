...
def unconfigured_hint() -> str:
    ...
    return ("邮件通知未配置（可选）：在 .env 填 MAIL_ADDRESS/MAIL_AUTH_CODE/MAIL_PROVIDER，"
...
def send_html(subject: str, html_body: str, *, to: str | None = None) -> dict:
    ...
    if not is_configured():
        return {"success": False, "skipped": True, "reason": "邮件未配置（MAIL_ADDRESS/MAIL_AUTH_CODE）"}
    ...
    try:
        if srv["ssl"]:
            with smtplib.SMTP_SSL(srv["host"], srv["port"], timeout=30,
...
def install_report_html(r: dict, src_dir) -> str:
    ...
    next_items = []
    if creds:
        next_items.append(f"配凭证：{', '.join(d['name'] for d in creds)}（见 install_list 的 credential_env）")
    if confs:
