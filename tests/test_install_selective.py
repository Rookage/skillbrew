...
def _build_source(tmp_path: Path, *, items: list[dict], decisions: list[dict],
                  recommend_judgments: list[dict] | None = None) -> Path:
...
def _stub_network(monkeypatch, db_path: Path):
    ...
    # stub fetch
    monkeypatch.setattr(install_mod, "_fetch_bytes",
                        lambda url: b"# fake skill\n")
...
def test_install_without_recommend_installs_all_new(tmp_path, monkeypatch):
    ...
    items = [
        {"name": "a", "form": "Skill", "dir_path": "skills/a",
         "files": [{"path": "skills/a/SKILL.md", "file_type": "md"}],
         "install_method": "per_file_raw_download"},
        {"name": "b", "form": "Skill", "dir_path": "skills/b",
    ...
    source = _build_source(tmp_path, items=items, decisions=decisions,
                           recommend_judgments=None)
...
def test_install_with_recommend_filters_to_approved(tmp_path, monkeypatch):
    ...
    items = [
        {"name": "good", "form": "Skill", "dir_path": "skills/good",
         "files": [{"path": "skills/good/SKILL.md", "file_type": "md"}],
         "install_method": "per_file_raw_download"},
        {"name": "bad", "form": "Skill", "dir_path": "skills/bad",
         "files": [{"path": "skills/bad/SKILL.md", "file_type": "md"}],
         "install_method": "per_file_raw_download"},
        {"name": "meh", "form": "Skill", "dir_path": "skills/meh",
    ...
    judgments = [
        {"name": "good", "decision": "new", "verdict": V_WORTH, "reason": "ok",
         "score": 8, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
        {"name": "bad", "decision": "new", "verdict": V_NOT_WORTH, "reason": "junk",
         "score": 2, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
        {"name": "meh", "decision": "new", "verdict": V_NOT_WORTH, "reason": "dupe-ish",
         "score": 4, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
    ]
    source = _build_source(tmp_path, items=items, decisions=decisions,
                           recommend_judgments=judgments)
    db_path = tmp_path / "home" / ".claude" / "skillbrew-registry.sqlite3"
    db_path.parent.mkdir(parents=True, exist_ok=True)
    target = tmp_path / "skills"
    _stub_network(monkeypatch, db_path)
...
def test_install_dry_run_recommend_stats(tmp_path, monkeypatch):
    ...
    items = [
        {"name": "x", "form": "Skill", "dir_path": "skills/x",
    ...
    judgments = [
        {"name": "x", "decision": "new", "verdict": V_WORTH, "reason": "ok",
         "score": 7, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
        {"name": "y", "decision": "new", "verdict": V_NOT_WORTH, "reason": "no",
         "score": 3, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
    ]
...
def test_merge_and_skip_bypass_recommend_filter(tmp_path, monkeypatch):
    ...
    items = [
        {"name": "keep", "form": "Skill", "dir_path": "skills/keep",
         "files": [{"path": "skills/keep/SKILL.md", "file_type": "md"}],
         "install_method": "per_file_raw_download"},
        {"name": "old_stuff", "form": "Skill", "dir_path": "skills/old",
    ...
    judgments = [
        {"name": "keep", "decision": "new", "verdict": V_WORTH, "reason": "ok",
         "score": 8, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
        # deprecated 的 item 即便 verdict 是值得装，也会被 deprecated 优先拦掉
        {"name": "old_stuff", "decision": "new", "verdict": V_WORTH, "reason": "kept",
         "score": 6, "signals": {}, "target": None, "mode": "ai", "form": "Skill",
         "usability": "ready"},
    ]
    source = _build_source(tmp_path, items=items, decisions=decisions,
                           recommend_judgments=judgments)
