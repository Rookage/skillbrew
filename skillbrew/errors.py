...
用法：
    from .errors import SkillbrewError, StepFailedError, ConfigError, NetworkError
    raise StepFailedError("字幕和视觉都为空，无法生成消化计划。"，
...
class StepFailedError(SkillbrewError):
    """管线步骤可恢复的失败（文件缺失、LLM 调用缺 key 等）。"""


StepFailed = StepFailedError  # 向后兼容别名
